...
async def handle_addstats_cmd(chat_id: int) -> None:
...
async def handle_admin_direction_selected(
    chat_id: int, message_id: int, checkpoint_id: str, direction: str
) -> None:
    logger.info(
        "handle_admin_direction_selected | chat_id=%s checkpoint=%s dir=%s",
...
async def handle_admin_reply(chat_id: int, text: str, reply_to_message: dict) -> bool:
    ...
    try:
        await get_supabase().table("time_stat").insert({
