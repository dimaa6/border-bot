...
A sorted set  "active_session_ids"  maps  chat_id (score=0) -> member=str(chat_id)
...
def session_exists(chat_id: int) -> bool:
    ...
    return get_redis().exists(_session_key(chat_id)) == 1
...
def get_session(chat_id: int) -> dict | None:
    ...
    """
    data = get_redis().hgetall(_session_key(chat_id))
...
def upsert_session(
    ...
    r.sadd(_INDEX_KEY, str(chat_id))
    logger.debug("upsert_session | chat_id=%s", chat_id)


def update_last_user_action(chat_id: int, timestamp_iso: str) -> None:
    """Update last_user_action_at for an existing session."""
    get_redis().hset(_session_key(chat_id), "last_user_action_at", timestamp_iso)
    logger.debug("update_last_user_action | chat_id=%s", chat_id)


def update_last_reminded(chat_id: int, timestamp_iso: str) -> None:
    """Update last_reminded_at for an existing session."""
    get_redis().hset(_session_key(chat_id), "last_reminded_at", timestamp_iso)
...
def get_all_sessions() -> list[dict]:
    ...
    """
    r = get_redis()
    chat_ids = r.smembers(_INDEX_KEY)
    sessions = []
